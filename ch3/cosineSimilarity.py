...
def computeSimilarity(band1, band2, userRatings):
    ...
    #"David" : {"Imagine Dragons": 3, "Daft Punk": 5, "Lorde": 4, "Fall Out Boy": 1}
       averages[key] = (float(sum(ratings.values())) #仅获取评分
                       / len(ratings.values())) #len在此处获取项目数
    num = 0  # numerator 分子
    dem1 = 0 # first half of denominator 分母
    dem2 = 0
    #获取用户和评分
    for (user, ratings) in userRatings.items():
       #并且这个用户被限定为同时对band1和band2评分的用户
       if band1 in ratings and band2 in ratings:
          #获取用户的所有物品评分的平均值(参照行57)
          avg = averages[user]
          #(R_ui - avg(R_u)) * (R_uj - avg(R_u)) <R_ui表示用户u对物品i的评分,同理得R_uj>
          #计算分子
          num += (ratings[band1] - avg) * (ratings[band2] - avg)
          #计算分母
          dem1 += (ratings[band1] - avg)**2
          dem2 += (ratings[band2] - avg)**2
    return num / (sqrt(dem1) * sqrt(dem2)) #返回余弦相似度
