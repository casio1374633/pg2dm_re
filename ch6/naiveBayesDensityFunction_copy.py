    ...
    def __init__(self, bucketPrefix, testBucketNumber, dataFormat):
            ...
            if i != testBucketNumber:
                ...
                for line in lines:
                    ...
                    for columnValue in nums:
                        ...
                        totals[category][col] += columnValue #数值型数据汇集累加
                        numericValues[category].setdefault(col, [])
                        numericValues[category][col].append(columnValue) #统计数值型数据的类别数
        #
        ...
        for (category, columns) in numericValues.items():
            ...
            for (col, values) in columns.items():
                ...
                columns[col] = 0
                self.ssd[category][col] = math.sqrt(SumOfSquareDifferences / (classes[category]  - 1)) #<-样本标准差,分类种数-1
    ###
    ##################################################
